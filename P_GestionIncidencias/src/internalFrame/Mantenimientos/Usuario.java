...
public class Usuario extends JInternalFrame implements ActionListener,
	...
	private JLabel lblEstado, lblArea, lblCodigo, lblNombre, lblApellidos,
			lblTipoDocumento, lblDni, lblarea, lblCorreo, lblTelefono,
			lblFechaDeIngreso, lblBCodigo;
	private JTextField txtNombre, txtApellidos, txtDNI, txtCorreo,
			txtTelefono, txtBusquedaCodigo;
	...
	private Usuario() {
		setFrameIcon(new ImageIcon(
				Usuario.class.getResource("/imagenesx16/user_16.png")));
...

		{
...

			{
					...
					panelListado.add(scrollPane);
					{
						...
						table.setModel(modelo = new DefaultTableModel(
								new Object[][] {}, new String[] { "Código",
										"Nombre", "Apellidos",
										"idTipoDocumento", "Doc. de Identidad",
										"codigo_Area", "Area", "Correo",
				...
				}
				{
					...
					}
					{
						...
						cboBArea.setModel(new DefaultComboBoxModel(
								areaController.lista()));
			...
			}
			{
				...
				tabbedPane.addTab("Mantenimiento", null, panelMantenimiento,
						null);
				...
				}
				{
					...
					cboTipoDocumento.setModel(new DefaultComboBoxModel(
							TDocumentoController.lista()));
					cboTipoDocumento.setBounds(110, 101, 300, 24);
				...
				}
				{
					...
					cboArea.setModel(new DefaultComboBoxModel(areaController
							.lista()));
				...
				}
				{
					...
					txtCodigo.setBounds(78, 12, 50, 20);
	...
	protected void do_btnRegistrar_actionPerformed(ActionEvent e) {
		...
		try {
			...
			if (tabbedPane.getSelectedIndex() == 0) {
				...
				if(leerApellidos().trim().isEmpty()){
					txtApellidos.setBorder(BorderFactory.createLineBorder(Color.RED,1));
					txtApellidos.requestFocus();
					return;
				}else
					txtApellidos.setBorder(BorderFactory.createLineBorder(Color.GRAY,1));
				if(leerDocumento().trim().isEmpty()){
					txtDNI.setBorder(BorderFactory.createLineBorder(Color.RED,1));
					txtDNI.requestFocus();
					return;
				}else
					txtDNI.setBorder(BorderFactory.createLineBorder(Color.GRAY,1));
				
				if (estado == "Guardar") {
					
					if(usuarioController.existsUserByDocument(leerDocumento())){
						JOptionPane.showMessageDialog(null, "El DNI ingresado ya existe. Usted ya se encuentra registrado");
						return;
					}	
					...
					if(usuarioController.existsUserByDocument(leerDocumento()) 
							&& !(table.getValueAt(table.getSelectedRow(), 4).equals(leerDocumento()))){
						JOptionPane.showMessageDialog(null, "El DNI ingresado ya existe. Usted ya se encuentra registrado");
						return;
					}	
					int codigo = Integer.parseInt(txtCodigo.getText());
					if (usuarioController.isUsuarioReferencedToIncidencia(codigo) && cboEstado.getSelectedIndex() == 1) {
						JOptionPane.showMessageDialog(null, "El Usuario está referenciada a una Incidencia. Por lo tanto, usted no puede cambiar su estado");
		...
		try {
			...
			if (General.validateJTableisEmpty(r, r))
				...
				int respuesta = JOptionPane.showConfirmDialog(null,
						"Está seguro de eliminar el usuario?", "advertencia",
						JOptionPane.YES_NO_OPTION,
						JOptionPane.QUESTION_MESSAGE, null);
				if (respuesta == JOptionPane.YES_OPTION) {
					int row = table.getSelectedRow();
					int codigo = (int) table.getValueAt(row, 0);
					if (usuarioController.isUsuarioReferencedToIncidencia(codigo)) {
						JOptionPane.showMessageDialog(null, "El Usuario está referenciada a una Incidencia. Por lo tanto, usted no puede cambiar su estado");
						return;
					}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (txtBusquedaCodigo.getText().isEmpty()) {
				...
				Object[] data = new Object[] {
						...
						u.getDocumento(),
						u.getCodigoArea(),
						usuarioController.getNombreAreaxCodigo(u
								.getCodigoArea()), u.getCorreo(),
						u.getTelefono(),
	...
	void carryDataToControls(int r) {
		...
		dateChooser.setDate(General.parseStringtoDate((String) table
				.getValueAt(r, 9)));
		cboEstado.setSelectedIndex((int) table.getValueAt(r, 10));
	}

	void addRowInserted() {
		modelo.addRow(new Object[] { leerCodigo(), leerNombre(),
				leerApellidos(), leerComboTipoDocumento(), leerDocumento(),
				leerCodigoArea(), leerArea(), leerCorreo(), leerTelefono(),
				leerFecha(), leerEstado() });
	}
	...
	void editRowSelected(BE_Usuario u) {
		...
		model.setValueAt(areaController.getNombreAreaxCodigo(u.getCodigoArea()), table.getSelectedRow(), 6);
		model.setValueAt(u.getCorreo(), table.getSelectedRow(), 7);
		model.setValueAt(u.getTelefono(), table.getSelectedRow(), 8);
		model.setValueAt(General.parsearDatetoString(u.getFechaIngreso()),
	...
	void fillTable() {
		...
		for (int i = 0; i < usuarioController.size(); i++) {
			...
			Object[] data = new Object[] { u.getCodigo(), u.getNombre(),
					u.getApellido(), u.getIdTipoDocumento(), u.getDocumento(),
					u.getCodigoArea(),
					usuarioController.getNombreAreaxCodigo(u.getCodigoArea()),
	...
	void fillTablexArea() {
		...
		for (int i = 0; i < aux.size(); i++) {
			...
			Object[] data = new Object[] { u.getCodigo(), u.getNombre(),
					...
					u.getDocumento(),
					u.getCodigoArea(),
					usuarioController.getNombreAreaxCodigo(u.getCodigoArea()),// u.getArea(),
					u.getCorreo(), u.getTelefono(),
					General.parsearDatetoString(u.getFechaIngreso()),
					u.getEstado() };
			model.addRow(data);
	...
	String leerDocumento() {
		if (!General.isNumeric(txtDNI.getText())
				|| General.isEmpty(txtDNI.getText())) {
			JOptionPane.showMessageDialog(null,
							"El DNI tiene caractáres inválidos. Ingrese sólo valores nómericos");
			return "";
		} else
	...
	void setDataUsuario(BE_Usuario objUsuario) {
		...
		objUsuario.setUsuario(leerNombre(), leerApellidos(), leerDocumento());
	}
	...
	// "El usuario seleccionado ya existe. Â¿Desea volver al formulario"
	// + "de incidencias?", "Aviso",JOptionPane.YES_NO_OPTION,
	...
	protected void do_table_mouseClicked(MouseEvent e) {
		// verifica si el formulario de incidencia ha abierto este formulario
		if (listener.equals("Incidencia")) {
			// si ha hecho double click en el jtable
...

			{
				...
				// pregunto si el mantenimiento es de actualización, de ser asi,
				// se actualiza el combo del formulario quien lo invoco
				// (En este caso el de Incidencia)s
				if (sender.equals("Editando")) {
					Incidencia.cboMUsuario.removeAllItems();
					for (String s : usuarioController.lista()) {
						Incidencia.cboMUsuario.addItem(s);
					...
					if (verifica(codigo, nombre)) {
						if (JOptionPane.showConfirmDialog(null,
								"El usuario seleccionado ya existe. ¿Desea volver al formulario "
										+ "de incidencias?", "Aviso",
								JOptionPane.YES_NO_OPTION,
								JOptionPane.WARNING_MESSAGE) == JOptionPane.YES_OPTION) {
							this.dispose();
							return;
