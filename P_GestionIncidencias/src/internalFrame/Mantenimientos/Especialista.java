...
public class Especialista extends JInternalFrame implements ActionListener {
	...
	private JLabel lblEstado, lblArea, lblCodigo, lblNombre, lblApellidos,
			lblEspecialidad, lblAnexo, lblFechaDeIngreso, lblBCodigo;
	private JTextField txtNombre, txtApellidos, txtEspecialidad,
			txtAnexo, txtBusquedaCodigo;
	...
	private Especialista() {
		...
		setFrameIcon(new ImageIcon(
				Especialista.class
						.getResource("/imagenesx16/especialista_16x16.png")));
		setClosable(true);
...

		{
...

			{
				...
				}
				{
					...
					{
						/*
						 * lblArea = new JLabel("Area:"); lblArea.setBounds(204,
						 * 32, 44, 13); panelFiltro.add(lblArea); } { cboBArea =
						 * new JComboBox(); cboBArea.setBounds(254, 27, 130,
						 * 24); cboBArea.setModel(new DefaultComboBoxModel(new
						 * String[]{ "Sistemas","Tu vieja" }));
		...
		try {
			if (tabbedPane.getSelectedIndex() == 0) {
				...
				if(leerNombre().trim().isEmpty())
				{
					...
					txtNombre.setBorder(LineBorder.createGrayLineBorder());
				if(leerApellidos().trim().isEmpty())
				{
					...
					txtApellidos.setBorder(LineBorder.createGrayLineBorder());
				if(leerEspecialidad().trim().isEmpty())
				{
					txtEspecialidad.setBorder(BorderFactory.createLineBorder(Color.RED,1));
					txtEspecialidad.requestFocus();
					return;
				}else
					txtEspecialidad.setBorder(LineBorder.createGrayLineBorder());
				...
				if (estado == "Guardar") {
					...
					if (EspecialistaController.isEspecialistaReferencedToIncidencia(codigo)&& cboEstado.getSelectedIndex() == 1) {
						JOptionPane.showMessageDialog(null,"El especialista está referenciado a una Incidencia. Por lo tanto, Usted no puede cambiar su estado");
	...
	protected void do_btnEliminar_actionPerformed(ActionEvent e) {
		try {
			...
			if (General.validateJTableisEmpty(r, r))
				...
				int respuesta = JOptionPane.showConfirmDialog(null,"¿Está seguro de eliminar el Especialista?", "advertencia",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE, null);
				if (respuesta == JOptionPane.YES_OPTION) {
					int row = table.getSelectedRow();
					int codigo = (int) table.getValueAt(row, 0);
					if (EspecialistaController.isEspecialistaReferencedToIncidencia(codigo)) {
						JOptionPane.showMessageDialog(null,"El especialista está referenciado a una Incidencia. Por lo tanto, Usted no puede cambiar su estado");
						return;
					}
					...
					EspecialistaController.eliminaEspecialista(codigo);
	...
	Action action = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (txtBusquedaCodigo.getText().isEmpty()) {
				...
				BE_Especialista u = EspecialistaController
						.listarEspecialistaxCodigo(codigo);
				Object[] data = new Object[] { u.getCodigo(), u.getNombres(),
						u.getApellidos(), u.getEspecialidad(), u.getAnexo(),
	...
	void addRowInserted() {
		modelo.addRow(new Object[] { leerCodigo(), leerNombre(),
				leerApellidos(), leerEspecialidad(), leerAnexo(), leerFecha(),
				leerEstado() });
	}
