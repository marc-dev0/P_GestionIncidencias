	...
	protected void do_btnRegistrar_actionPerformed(ActionEvent e) {
		if (tabbedPane.getSelectedIndex() == 0) {
			...
			if (estado == "Guardar") {
				...
				if (TipoIncidenciaController.isTIncidenciaReferencedToIncidencia(codigo)&& cboEstado.getSelectedIndex() == 1) {
					JOptionPane.showMessageDialog(null, "El Tipo de Incidencia está referenciado a una Incidencia. Por lo tanto, Usted no puede cambiar su estado");
		...
		try {
			...
			if (General.validateJTableisEmpty(rowcount, rowSelected))
				...
				if (JOptionPane.showConfirmDialog(null, "Desea eliminar el registro",
						"Adveretencia", JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {
					BE_TipoIncidencia a = new BE_TipoIncidencia();
					int codigo = (int) table.getValueAt(rowSelected, 0);
					if (TipoIncidenciaController.isTIncidenciaReferencedToIncidencia(codigo)) {
						JOptionPane.showMessageDialog(null, "El Tipo de Incidencia está referenciado a una Incidencia. Por lo tanto, Usted no puede cambiar su estado");
						return;
					}
					a = TipoIncidenciaController.buscarTipoIncidenciaxCodigo(codigo);
					TipoIncidenciaController.eliminarTipoIncidencia(a);
					TipoIncidenciaController.eliminaTipoIncidencia(codigo);
	...
	Action action = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (txtBusquedaCodigo.getText().isEmpty()) {
				...
				BE_TipoIncidencia a = new BE_TipoIncidencia();
				a = TipoIncidenciaController
						.buscarTipoIncidenciaxCodigo(codigo);
