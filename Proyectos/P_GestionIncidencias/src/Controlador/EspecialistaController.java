...
public class EspecialistaController {
		...
		try {
			...
			if(fEspecialista.exists()){
				...
				while((linea = br.readLine())!= null){
					...
					arrayEspecialista.add(new BE_Especialista(codigo, nombres, apellidos, especialidad, 
																  anexo, fechaIngreso, estado));
				}
			br.close();
			}else
				return;
	...
	public String[] lista(){
		...
		for (int i = 0 ; i< size() ; i++){
			BE_Especialista e = arrayEspecialista.get(i);
			data[i] = e.getNombres();
		}
		return data;
	}
